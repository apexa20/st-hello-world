...
try:
 model = joblib.load(r"C:\Users\dhara\Desktop\delivery_time_model.pkl") 
...
product_category = st.sidebar.selectbox(" Select Product Category", ["Electronics", 
"Clothing", "Furniture", "Books", "Others"]) 
customer_location = st.sidebar.selectbox(" Select Customer Location", ["Urban", 
"Suburban", "Rural"]) 
shipping_method = st.sidebar.selectbox(" Select Shipping Method", ["Standard", "Express", 
"Same-Day"]) 
...
if st.sidebar.button(" Predict Delivery Time"): 
    try: 
        ...
        chart_data = pd.DataFrame({"Delivery Estimate": ["Predicted Delivery Time"], "Days": 
[predicted_time]}) 
        chart = alt.Chart(chart_data).mark_bar(size=50).encode( 
            x=alt.X("Days:Q", title="Estimated Days", scale=alt.Scale(domain=(0, predicted_time + 
2))), 
